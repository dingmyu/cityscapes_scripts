...
def get_parser():
    ...
    parser.add_argument('--val_list1', type=str, default='/mnt/lustre/share/dingmingyu/cityscapes/instance_list_new.txt', help='val list')
...
def main():
    ...
    fff = open('/mnt/lustre/share/dingmingyu/cityscapes/instance_list_new.txt').readlines()
    flag = []
...
def validate(val_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, flag):
    ...
    for i, (input, label1, label2) in enumerate(val_loader):
        ...
        print (output.shape)
        cv2.imwrite('result/result_%d.png' % i, output[:,:,1]*255)
