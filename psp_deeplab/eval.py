...
from helper.load_helper import restore_from2
...
def get_parser():
    ...
    parser.add_argument('--crop_h', type=int, default=713, help='validation crop size h')
    parser.add_argument('--crop_w', type=int, default=713, help='validation crop size w')
...
def main():
    ...
    if not args.has_prediction:
        model = PSPNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, use_softmax=False,
                       use_aux=False, pretrained=False, syncbn=syncbn, group_size=bn_group,
                   group=bn_group_comm, sync_stats=bn_sync_stats).cuda()
        logger.info(model)
        ...
        if os.path.isfile(args.model_path):
            ...
            model, epoch = restore_from2(model, args.model_path)
            # checkpoint = torch.load(args.model_path)
            # model.load_state_dict(checkpoint['state_dict'], strict=False)
            logger.info("=> loaded checkpoint '{}'".format(args.model_path))
        else:
            raise RuntimeError("=> no checkpoint found at '{}'".format(args.model_path))
        validate(val_loader, val_data.data_list, model, args.classes, mean, std, args.base_size, args.crop_h, args.crop_w, args.scales, gray_folder=gray_folder)
    if args.split is not 'test':
        cal_acc(val_data.data_list, gray_folder, args.classes)
...
def validate(val_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder):
