...
from utils import AverageMeter, poly_learning_rate, intersectionAndUnion, simple_group_split


from helper.distributed_utils import dist_init, average_gradients, broadcast_params
from helper.lr_helper import IterExponentialLR
from torch.utils.data.distributed import DistributedSampler
import torch.distributed as dist
from torch.optim.lr_scheduler import MultiStepLR
from helper.load_helper import restore_from, load_pretrain
...
def get_parser():
    ...
    parser.add_argument('--crop_h', type=int, default=713, help='train crop size h')
    parser.add_argument('--crop_w', type=int, default=713, help='train crop size w')
...
def main():
    ...
    if args.bn_group > 1:
        args.syncbn = True
        bn_sync_stats = True
        bn_group_comm = simple_group_split(world_size, rank, world_size // args.bn_group)
    else:
        args.syncbn = False
        bn_sync_stats = False
        bn_group_comm = None
    ...
    model = PSPNet(layers=args.layers, classes=args.classes,
                   zoom_factor=args.zoom_factor, syncbn=args.syncbn, group_size=args.bn_group,
                   group=bn_group_comm, sync_stats=bn_sync_stats)
    if rank == 0:
        logger.info(model)
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=False if train_sampler else True, num_workers=args.workers, pin_memory=False, sampler=train_sampler)
    ...
    if args.evaluate:
        ...
        val_loader = torch.utils.data.DataLoader(val_data, batch_size=args.batch_size_val, shuffle=False, num_workers=args.workers, pin_memory=False, sampler=val_sampler)
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if epoch % args.save_step == 0 and (rank == 0):
            ...
            torch.save({'epoch': epoch, 'state_dict': model.cpu().state_dict(), 'optimizer': optimizer.state_dict()}, filename)
...
def train(train_loader, model, criterion, optimizer, epoch, zoom_factor, batch_size, aux_weight):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss = (main_loss + aux_weight * aux_loss) / world_size
        ...
        if (i + 1) % args.print_freq == 0 and rank == 0:
            logger.info('Epoch: [{}/{}][{}/{}] '
