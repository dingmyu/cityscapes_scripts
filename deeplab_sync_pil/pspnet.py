...
class PPM(nn.Module):
    def __init__(self, in_dim, reduction_dim, bins, BatchNorm=nn.BatchNorm2d):
        ...
        self.features.append(nn.Sequential(
            ...
            nn.Upsample(size=(90, 90), mode='bilinear', align_corners=True),
            nn.Conv2d(in_dim, 256, kernel_size=1, stride=1, bias=False),			
            BatchNorm(256),
            nn.ReLU(inplace=True)
        ))
...
class PSPNet(nn.Module):
    def __init__(self, layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=8, use_softmax=True, use_aux=True, pretrained=True, syncbn=True, group_size=1, group=None, sync_stats=False):
        ...
        if syncbn:
            ...
            def BNFunc(*args, **kwargs):
                return SyncBatchNorm2d(*args, **kwargs, eps=1e-4, momentum=0.9, group_size=group_size, group=group, sync_stats=sync_stats)
            BatchNorm = BNFunc
        else:
    ...
    def forward(self, x):
        x_size = x.size()
        assert (x_size[2]-1) % 8 == 0 and (x_size[3]-1) % 8 == 0, "x_size[2] is %d, x_size[3] is %d" % (x_size[2], x_size[3])
